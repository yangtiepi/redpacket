...
public class CardsServiceImpl implements ICardsService{
	...
	@Override
	public Map<String, Object> cardsComplete(Long cardsId, String openid,String nickname) {
		CardQuery qu = new CardQuery();
		qu.setCardsId(cardsId);
		qu.setPageSize(-1);
		List<Card> cardList = cardMapper.query(qu);
		UserCardQuery equ = new UserCardQuery();
		equ.setCardId(cardsId);
		equ.setOpenid(openid);
		equ.setPageSize(-1);
		List<UserCard> userCardList = userCardMapper.query(equ );
		int size = cardList.size();
		for (Card card : cardList) {
			for (UserCard userCard : userCardList) {
				if (card.getId().equals(userCard.getCardId())) {
					int delete = userCardMapper.delete(userCard.getId());
					if (delete == 1) {
						size--;
					}
				}
